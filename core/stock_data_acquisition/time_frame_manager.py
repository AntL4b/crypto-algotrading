...
from core.stock_data_acquisition.stock_data_manager import StockDataManager, MAX_ITEM_IN_DATA_SET
...
class TimeFrameManager(object):
    ...
    def __init__(self, time_frame_length: int, market: str, rest_api: ExchangeRestApi,
        ...
        :param rest_api: Instance of ExchangeRestApi
        ...
        self._rest_api: ExchangeRestApi = rest_api
    ...
    def feed(self) -> None:
        ...
        response: [StockDataDict] = self._feed()
    ...
    def _feed(self) -> [StockDataDict]:
        ...
        while True:
            try:
                ...
                logging.error(
                    f"REST API: Http request failed, trying again in {retry_delay} sec. Details: {str(rest_api_ex)}")
                time.sleep(retry_delay)
            except KeyError as key_err:
                logging.error(
                    f"REST API: Data format error, trying again in {retry_delay} sec. Details: {str(key_err)}")
                time.sleep(retry_delay)
            except Exception as e:
                logging.error(f"REST API: Unknown error, trying again in {retry_delay} sec. Details: {str(e)}")
                time.sleep(retry_delay)
            finally:
